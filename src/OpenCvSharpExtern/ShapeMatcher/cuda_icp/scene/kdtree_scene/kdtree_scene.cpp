...
{
    assert(pcd_buffer.size() > 0 && pcd_buffer.size() == normal_buffer.size() && "no pcd yet, or pcd size != normal size");
    ...
    { // when we have new nodes to split, go
        ...
        for (size_t node_iter = num_nodes_last_last_turn; node_iter < num_nodes_now_last_turn; node_iter++)
        {
            ...
            if (nodes[node_iter].right - nodes[node_iter].left > max_num_pcd_in_leaf)
            {
                ...
                // get bbox
                float x_min = FLT_MAX;
                float x_max = -FLT_MAX; // fxxk, FLT_MIN is 0
                float y_min = FLT_MAX;
                float y_max = -FLT_MAX;
                ...
                span_xyz[0] = x_max - x_min;
                split_v_xyz[0] = (x_min + x_max) / 2;
                span_xyz[1] = y_max - y_min;
                split_v_xyz[1] = (y_min + y_max) / 2;
                ...
                nodes[node_iter].bbox[0] = x_min;
                nodes[node_iter].bbox[1] = x_max;
                nodes[node_iter].bbox[2] = y_min;
                nodes[node_iter].bbox[3] = y_max;
