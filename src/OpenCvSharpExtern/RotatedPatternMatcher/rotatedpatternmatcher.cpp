...
void RotatedPatternMatcher::teach(cv::Mat *pattern)
{
    ...
    int iTopLayer = m_iPyramidLevels > 0 ? m_iPyramidLevels - 1 : 0;
    ...
    if (!templData->vecPyramid.empty())
    {
        const auto &topLayer = templData->vecPyramid.back();
        std::cout << "Smallest pyramid size: " << topLayer.cols << "x" << topLayer.rows << std::endl;
    }
...
std::vector<RotationPatternMatcherResults> RotatedPatternMatcher::search(cv::Mat *image)
{
    ...
    int iTopLayer = m_TemplData.vecPyramid.size() - 1;
    if (iTopLayer < 0)
        return results;

    const auto &topTemplateLayer = m_TemplData.vecPyramid.back();
    std::cout << "Smallest pyramid size: " << topTemplateLayer.cols << "x" << topTemplateLayer.rows << std::endl;
